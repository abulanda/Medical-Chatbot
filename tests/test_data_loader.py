...
def test_exact_match(tmp_path):
    ...
    results = loader.find_diseases_by_symptoms(
        ["fever", "cough"], min_hits=1.0, top_k=3
    )
...
def test_fuzzy_match(tmp_path):
    ...
    loader = DataLoader(Path(path))
    results = loader.find_diseases_by_symptoms(
        ["feever", "head ache"], min_hits=1.0, top_k=3
    )
...
def test_negation_changes_ranking(tmp_path):
    ...
    loader = DataLoader(Path(path))
    results = loader.find_diseases_by_symptoms(
        ["no cough", "fever"], min_hits=1.0, top_k=3
    )
    assert results
...
def test_weights_and_min_hits(tmp_path):
    ...
    loader = DataLoader(Path(path))
    results = loader.find_diseases_by_symptoms(
        ["fever"], min_hits=2.0, top_k=5, symptom_weights={"fever": 2.0}
    )
    assert results
